...
size_t Network::random_connect(const double& lamda){
	...
	for (size_t node=0; node < size(); ++node){
		...
		size_t myDegree (RNG.poisson(lamda)); 	//we add myDegree new links to the node (add to the existing)
		
		for(size_t i = 0; i < myDegree; ++i){
			
			size_t newConnectionPoint(node);
			
			while ((newConnectionPoint == node)) { 	//if the link is already established it shouldn't do it again or can there be several links between two nodes?
													//check with same loop as in Network::add_link, but then theres a problem if I want do do mor links than number of free/possible nodes 
